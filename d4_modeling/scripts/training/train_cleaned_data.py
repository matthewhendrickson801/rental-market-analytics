"""
Train XGBoost with cleaned team data
Goal: Maintain high R² while reducing overfitting
"""
...
print("=" * 80)
print("TRAINING XGBOOST WITH CLEANED DATA")
print("Goal: High R² with less overfitting")
print("=" * 80)
...
exclude_cols = [
    ...
    'Boom_Category',
    # Keep log version, remove original skewed version
    'jobs_per_capita'
]
...
X_train, X_val, y_train, y_val = train_test_split(
    X_temp, y_temp, test_size=0.1875, random_state=42, stratify=df.loc[X_temp.index, 'city']
)
...
print("\n5. Training XGBoost...")
print("   Hyperparameters (with stronger regularization):")
params = {
    'objective': 'reg:squarederror',
    'max_depth': 5,  # Reduced from 6
    'learning_rate': 0.03,  # Reduced from 0.05
    'n_estimators': 400,  # Increased for slower learning
...
if train_val_gap < 0.05:
    ...
    print("⚠️  Moderate overfitting (gap 0.05-0.10)")
else:
...
if val_test_gap < 0.02:
    ...
    print("⚠️  Validation and test performance differ")
...
# Save model
print("\n9. Saving model...")
model_path = MODELS_DIR / 'xgboost_cleaned.pkl'
with open(model_path, 'wb') as f:
    pickle.dump(model, f)
print(f"   Model saved to: {model_path}")

# Save scaler
scaler_path = MODELS_DIR / 'scaler_cleaned.pkl'
with open(scaler_path, 'wb') as f:
    pickle.dump(scaler, f)
print(f"   Scaler saved to: {scaler_path}")

# Save metrics
metrics_path = RESULTS_DIR / 'metrics' / 'xgboost_cleaned_metrics.json'
metrics = {
    'train': train_metrics,
...
fi_path = RESULTS_DIR / 'metrics' / 'xgboost_cleaned_feature_importance.csv'
